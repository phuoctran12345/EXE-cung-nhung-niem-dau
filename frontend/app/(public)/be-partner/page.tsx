...
export const STORAGE_KEY = "partner_application_draft";
...
export default function BecomePartnerPage() {
  ...
  const handleLicenseUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!token) {
      setError("Vui lòng đăng nhập trước khi tải giấy phép kinh doanh.");
      return;
    }
  ...
  const handleNext = () => {
    ...
    const token = localStorage.getItem("token");
    if (!token) {
      setError("Bạn cần đăng nhập để đăng ký trở thành đối tác.");
      return;
    }
    if (!formData.companyName || !formData.taxCode || !formData.address) {
    ...
    sessionStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ ...formData, licenseUrl }),
    );
    router.push("/be-partner/contract");
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] pt-12 pb-24 px-4">
      <div className="max-w-[1000px] mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-[40px] font-black text-[#1E293B] mb-4">
            Trở thành Đối tác Travel Match
          </h1>
        ...
        <div className="bg-white rounded-[40px] shadow-xl shadow-blue-900/5 p-8 md:p-12 border border-gray-100 flex flex-col md:flex-row gap-12">
          <div className="flex-1 space-y-8">
            <h2 className="text-2xl font-bold text-[#1E293B] flex items-center gap-3">
              <span className="w-8 h-8 rounded-lg bg-blue-50 text-[#38BDF8] flex items-center justify-center text-sm">
                01
              </span>
          ...
          <div className="w-full md:w-[350px] space-y-8">
            ...
            <button
              ...
              className="w-full p-8 bg-blue-50/50 rounded-[32px] border-2 border-dashed border-blue-200 flex flex-col items-center text-center group hover:border-[#38BDF8] transition-all cursor-pointer"
            >
              ...
              <p className="text-[13px] text-gray-500 font-medium">
                {licenseUrl ? "Đã tải lên thành công" : "Hỗ trợ: PDF, JPG, PNG (Tối đa 10MB)"}
              </p>
              <span className="mt-6 bg-white text-[#38BDF8] px-6 py-2 rounded-xl font-bold text-sm shadow-sm border border-blue-100">
                {uploading ? "Đang tải..." : licenseUrl ? "Đổi file" : "Chọn file"}
              </span>
            </button>
            ...
            <div className="space-y-4 pt-4">
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  ...
                  className="mt-1.5 rounded text-[#38BDF8] focus:ring-[#38BDF8]"
                />
                <span className="text-[14px] text-gray-500 font-medium leading-relaxed group-hover:text-[#1E293B] transition-colors">
                  Tôi đồng ý với{" "}
                  <span className="text-[#38BDF8] font-bold underline">Điều khoản dịch vụ</span>{" "}
                  và{" "}
                  <span className="text-[#38BDF8] font-bold underline">Chính sách bảo mật</span>{" "}
                  của Travel Match.
                </span>
              </label>
              ...
              <p className="text-center text-[12px] text-gray-400 font-medium">
                Bạn cần đăng nhập để gửi hồ sơ cho Admin duyệt.
              </p>
            </div>
          </div>
        </div>
...
function StepItem({
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-all ${active ? "bg-[#38BDF8] text-white shadow-lg shadow-[#38BDF8]/30 scale-110" : "bg-white text-gray-400 border-2 border-gray-100"}`}
      >
        {number}
      </div>
      <span className={`font-bold text-[15px] ${active ? "text-[#1E293B]" : "text-gray-400"}`}>
        {label}
      </span>
    </div>
  );
